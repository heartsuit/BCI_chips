function algo_test_EP_double_cor(class,chan,csp,Fe,filtering,N_start,N_end)
%=====================================================
...
for i=1:nbclasses
    ...
    if i~=2
        for j=1:nb_trials
            ...
            class(i).signal_corr_2(j,:) = xcorr(class(i).signal_SpatFilter(j,:),class(2).signal_mean);
            class(i).signal_corr_1(j,:) = xcorr(class(i).signal_SpatFilter(j,:),squeeze(mean(class(i).signal_SpatFilter(ind_moy,:),1)));
        end
    else
        for j=1:nb_trials
            ind_moy = [1:j-1 j+1:nb_trials];
            class(i).signal_corr_1(j,:) = xcorr(class(i).signal_SpatFilter(j,:),class(1).signal_mean);
            class(i).signal_corr_2(j,:) = xcorr(class(i).signal_SpatFilter(j,:),squeeze(mean(class(i).signal_SpatFilter(ind_moy,:),1)));
        end
    end
...
taux_max = max(max(class(2).signal_corr_2));
taux_min = min(min(class(2).signal_corr_2));
ori = round(size(class(1).signal_corr_2,2)/2+1)+(-100:100);
...
for taux=taux_h
    i=i+1;
    detect(1,i) = 100*sum(max(class(1).signal_corr_2(:,ori),[],2) >= taux)/size(class(1).signal_corr_2,1);
    detect(2,i) = 100*sum(max(class(2).signal_corr_2(:,ori),[],2) >= taux)/size(class(2).signal_corr_2,1);
end
...
for taux = taux_b
    i=i+1;
    detect2(2,i) = 100*sum(max(class(1).signal_corr_2(:,ori),[],2) <= taux)/size(class(1).signal_corr_2,1);%(sum(max(class(1).signal_corr(:,ori),[],2) <= taux) + sum(max(class(2).signal_corr(:,ori),[],2) <= taux));
    detect2(1,i) = 100*sum(max(class(2).signal_corr_2(:,ori),[],2) <= taux)/size(class(2).signal_corr_2,1);%(sum(max(class(1).signal_corr(:,ori),[],2) <= taux) + sum(max(class(2).signal_corr(:,ori),[],2) <= taux));
end
figure
hold on
...
ind_seuil = find(detect(1,:) <= 50);
[max_value,ind_max] = max(detect(2,ind_seuil));
seuil = taux_h(ind_seuil(ind_max));
disp(['seuil : ' num2str(seuil)]);
nb_class_1_peak = sum(max(class(1).signal_corr_2,[],2) >= seuil & (max(class(1).signal_corr_2(:,ori),[],2)./max(class(1).signal_corr_1(:,ori),[],2) > 5));
nb_class_2_peak = sum(max(class(2).signal_corr_2,[],2) >= seuil & (max(class(2).signal_corr_2(:,ori),[],2)./max(class(2).signal_corr_1(:,ori),[],2) > 5));
nb_class_1_no_peak = sum(max(class(1).signal_corr_2,[],2) < seuil);
nb_class_2_no_peak = sum(max(class(2).signal_corr_2,[],2) < seuil);


disp(num2str(nb_class_1_peak))
disp(num2str(nb_class_2_peak))
disp(num2str(nb_class_1_no_peak))
disp(num2str(nb_class_2_no_peak))
