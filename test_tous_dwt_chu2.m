...
%     N_dec = 3;
    ...
    for ii=1:nb_subsets
        ...
        [xapp,U,x_mean,x_std] = function_acp2(features_reshape_bb_app,y_bb_app);
        xapp = xapp(:,1:nbvarpca);
        m=size(features_reshape_bb_test,1);
        xtest_centre = (features_reshape_bb_test - x_mean(ones(m,1),:))./x_std(ones(m,1),:);
        xtest = xtest_centre*U;
        xtest = xtest(:,1:nbvarpca);
        ...
        y_estim_bb = [y_estim_bb; test_class_ovr(xtest, sep_bb, nb_class, xapp, y_bb_app, kernelparam)];
